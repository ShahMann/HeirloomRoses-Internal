...
if (!customElements.get("variant-picker")) {
  class VariantPicker extends HTMLElement {
    ...
    updatePrice() {
      ...
      if (this.preSelection) {
        ...
        for (let i = 1; i < this.data.product.variants.length; i += 1) {
          if (this.data.product.variants[i].price < variant.price)
      ...
      if (this.variant) {
        ...
        this.price.classList.toggle(
          "price--sold-out",
          !this.variant.available && !this.preSelection
        );
      }
    ...
    updateUrl(evt) {
      ...
      console.log("Variant changed, URL updated");

      if (this.variant) {
        const newPrice = this.variant.price / 100;
        window.dispatchEvent(
          new CustomEvent("tabby:priceUpdate", {
            detail: {
              price: newPrice,
            },
          })
        );
        console.log("Tabby price updated 123:", newPrice);
        window.dispatchEvent(
          new CustomEvent("tamara:priceUpdate", {
            detail: {
              price: newPrice,
            },
          })
        );
        console.log("Tamara price updated 123:", newPrice);
      }
    }
